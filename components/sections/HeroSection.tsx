...
const tiers = [
  ...
  { stake: "₦5K", win: "₦1 Million", color: "from-blue-600 to-blue-500" },
];
...
export default function HeroSection1() {
  return (
    <section className="relative bg-[#0B0E14] overflow-hidden">
      ...
      <div className="relative max-w-md mx-auto px-6 pt-10 pb-12 text-center">
        {/* Eyebrow badge */}
        <div className="inline-flex items-center gap-1.5 rounded-full border border-amber-400/30 bg-amber-400/10 px-4 py-1.5 text-xs font-semibold tracking-wide text-amber-400 mb-6">
          <Sparkles className="h-3.5 w-3.5" />
          WIN UP TO ₦1 MILLION
        </div>

        {/* Headline */}
        <h1 className="text-5xl leading-[1.05] font-black text-white mb-8">
          Pay small.{" "}
        ...
        <div className="space-y-3 mb-6">
          {tiers.map((tier) => (
            <button
              key={tier.stake}
              className={`w-full flex items-center justify-between rounded-2xl bg-gradient-to-r ${tier.color} px-6 py-4 shadow-lg shadow-black/20 transition-transform hover:scale-[1.02] active:scale-[0.98]`}
            >
        ...
        {/* Primary CTA */}
        <button className="w-full rounded-2xl bg-gradient-to-r from-amber-300 via-amber-400 to-amber-500 px-6 py-4 font-bold text-[#1a1305] text-base flex items-center justify-center gap-2 shadow-lg shadow-amber-500/20 transition-transform hover:scale-[1.02] active:scale-[0.98]">
          Start winning now
        ...
        <div className="mt-6 flex items-center justify-center flex-wrap gap-2">
          <span className="inline-flex items-center gap-1.5 rounded-full bg-white/5 border border-white/10 px-3 py-1.5 text-xs text-gray-300">
